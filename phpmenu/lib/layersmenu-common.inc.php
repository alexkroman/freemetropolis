...
function LayersMenuCommon() {
	...
	$this->author = 'Marco Pratesi (marco at marcopratesi dot it)';
...
/* END BENCHMARK CODE */
	...
	unset($dbresult);
	unset($row);
	$this->_depthFirstSearch($this->_tmpArray, 1, 1);
/* BEGIN BENCHMARK CODE
...
* @return void
*/
function _depthFirstSearch($tmpArray, $parent_id=1, $level) {
	reset ($tmpArray);
	while (list($id, $foobar) = each($tmpArray)) {
		if ($foobar['parent_id'] == $parent_id) {
			...
			unset($foobar);
			if ($id != $parent_id) {
				$this->_depthFirstSearch($this->_tmpArray, $id, $level+1);
			}
		}
	}
}
